...
export class AgenticsProdStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const subnetGroup = new elasticache.CfnSubnetGroup(this, 'RedisSubnetGroupProd', {
      description: 'Subnet group for Redis',
      subnetIds: vpc.selectSubnets({ subnetType: ec2.SubnetType.PRIVATE_WITH_EGRESS }).subnetIds,
      cacheSubnetGroupName: 'agentics-redis-subnet-group-prod',
    });
    // 1. Security Group
    const redisSG = new ec2.SecurityGroup(this, 'RedisSGProd', {
      vpc,
      description: 'Allow ECS to access Redis Replication Group',
    });

    // 3. Redis Replication Group (primary + one replica) with failover
    const redisRG = new elasticache.CfnReplicationGroup(this, 'RedisReplicationGroupProd', {
    ...
    // const redisReplicaTarget = new appscaling.CfnScalableTarget(this, 'RedisReplicaScalableTargetProd', {
    ...
    // #########################################################
    // 1. Fargate Service
    const fastApiService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, 'FastApiProdService', {
      cluster,
      cpu: 256,
      memoryLimitMiB: 512,
      desiredCount: 1,
      ...
      taskImageOptions: {
        image: ecs.ContainerImage.fromEcrRepository(
          ecr.Repository.fromRepositoryName(
            this,
            'FastApiRepoProd',
            this.node.tryGetContext('ecrRepository') ?? 'agentics/fastapi-prod'
          ),
          'latest'
        ),
        containerPort: 8001,
    ...
    // e.g. `npx cdk deploy AgenticsProdStack -c certificateArn=arn:aws:acm:...`.
    ...
    // #########################################################
    // Outputs
    new CfnOutput(this, 'FastApiEcsClusterProdNameOutput', {
      value: cluster.clusterName,
      exportName: 'FastApiEcsClusterProdName',
    });

    new CfnOutput(this, 'FastApiServiceNameProdOutput', {
      value: fastApiService.service.serviceName,
      exportName: 'FastApiServiceProdName',
    });

    new CfnOutput(this, 'FastApiLoadBalancerDNSProdOutput', {
      value: fastApiService.loadBalancer.loadBalancerDnsName,
      exportName: 'FastApiLoadBalancerDNSProd',
    });
